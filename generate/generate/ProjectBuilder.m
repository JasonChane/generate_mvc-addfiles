...
+ (PBXBuildFile*)buildFileWith:(NSString*)fileName fileRef:(NSString*)fileRef
{
    ...
    buildFile.key = [FFXCProjectUIDGenerator randomXcodeProjectUIDWithLength:24];
...
+ (id)groupWithName:(NSString*)name childName:(NSString*)childName type:(NSInteger)count childrenKeys:(NSArray*)childrenKeys
{
    NSMutableArray *childen = [NSMutableArray array];
    childen = [NSMutableArray arrayWithArray:childrenKeys[count + 1]];//childrenKeys比count多一位
    
    PBXGroup *group = [[PBXGroup alloc]init];
    group.isa = @"PBXGroup";
    group.name = name;
    group.key = childrenKeys[count][0];
    group.children = childen;
