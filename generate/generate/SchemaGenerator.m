...
- (BOOL)generate//组织数据
{
    ...
    OriginDataModel *originDataModel = [OriginDataModel shareInstance];//工程文件model
    Cus_Generate_Model* sourceModel = [Cus_Generate_Model shareInstance];//源配置文件
    
    inputFullPath = [NSString stringWithFormat:@"%@/%@", sourceModel.inputPath, sourceModel.inputFile];//读取json文件
    ...
    if (sourceModel.outPutPath && ![sourceModel.outPutPath isEqualToString:@""])
    {//若自定义了输出路径则把文件放在自定义的输出路径
//        outputPath = model.outPutPath;
    }
    //创建路径
    NSString *vcPath = [NSString stringWithFormat:@"%@/Controller/%@", outputPath,sourceModel.moduleName];
    NSString *viewPath = [NSString stringWithFormat:@"%@/View/%@", outputPath,sourceModel.moduleName];
    NSString *view_cellPath = [NSString stringWithFormat:@"%@/View/%@/Cell", outputPath,sourceModel.moduleName];
    NSString *themePath = [NSString stringWithFormat:@"%@/Theme/ThemeVO/%@", outputPath,sourceModel.moduleName];
    ...
    if (sourceModel.generateType == 0)
    {
        NSString *proName = sourceModel.vcName;
//        proName = [NSString stringWithFormat:@"%@List",proName];
    ...
    else if (sourceModel.generateType == 1)
    {
        NSString *proName = sourceModel.vcName;
        NSString *hh;
        NSString *mm;
